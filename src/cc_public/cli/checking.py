...
description:            |
                        The check command adapts its options into a
                        judgement for the driver where evals are asked
                        for. Where --changed-since is given, it
                        narrows the judgement to the items in the
                        files changed since that commit. It writes the
                        report in the format asked for and turns the
                        report into an exit status: two for a failure
                        of the analysis, always, and one for a
                        critical finding, only when asked. The format
                        command lays every document out, or says what
                        it would change.
relation:               []
