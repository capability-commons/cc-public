...
def test_the_dossier_projects_the_specimens_and_renders_both_documents(tmp_path):
    ...
    report = {'report': {'check': [{'id_check': 'eval', 'nonconformity': [
        {'filepath': str(ROOT / 'eval' / 'evl_req_singular.yaml'),
         'path':     'req_frontline_solar_buffer_kit_environmental_resilience',
    ...
    assert len(promoted) == 1 and len(promoted[0]['requirement']) >= 3
    assert all(3 <= len(c['candidate']) <= 12 for c in d['concept'])
    req = next(r for r in d['requirement']
               if r['id'] == 'req_frontline_solar_buffer_kit_environmental_resilience')
    ...
    assert 'In five moves' in briefing and need['title'] in briefing and '<script' not in briefing
