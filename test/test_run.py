...
def deploy(repo, **kw):
    ...
    for (k, v) in kw.items():
        cc_public.edit.field.set_field(tree, 'dep_record_from_schema_local', k, value = v)
...
def test_dry_run_plans_and_writes_nothing(repo):
    ...
    r = cc_public.workflow.run.run(repo, 'wf_record_from_schema',
                                   'dep_record_from_schema_local', BIND,
                                   None, None, is_dry = True)
    assert r['order'] == ['draft', 'review']
    assert r['node'][1]['output'] == {'record': 'revises draft'}
...
def test_run_makes_then_revises_and_commits(repo):
    ...
    assert draft['verdict']['record'] == [('evl_prose_matches_structure', 'met')]
    ...
    assert 'prt_draft_record.subject' in ports and 'prt_review_record.record' in ports
    guids = {b['relation'][0]['guid_target'] for b in exe['binding'].values()
             if b['id_port'] in ('prt_draft_record.record', 'prt_review_record.record')}
    assert len(guids) == 1                                 # revised in place

    assert clean(repo) == []
    c = next(cc_public.load.git.iter_commit(repo, 1))
    assert c.document['title'].startswith('Run wf_record_from_schema')
...
def test_guard_declines_and_run_stops_and_restores(repo):
    ...
    tree = cc_public.edit.tree.Tree([repo])
    cc_public.edit.field.set_field(tree, 'wf_record_from_schema',
                                   'edge."draft.output.record".0.guard', value = 'met')
...
def test_refuses_dirty_tree_when_committing(repo):
    ...
    with pytest.raises(cc_public.workflow.run.Stop):
        cc_public.workflow.run.run(repo, 'wf_record_from_schema',
                                   'dep_record_from_schema_local', BIND,
                                   Scripted(), Judge())
